...
public class Grille implements GrilleLectureSeule {
	...
	public Jeton ajouterJeton(int idColonne, int idRangee, Couleur couleur) {
		J.appel(this);
		
		return colonnes.get(idColonne).ajouterJeton(idRangee, couleur);
	}
	
	public void retirerJeton(int idColonne, int idRangee) {
		J.appel(this);
		
		colonnes.get(idColonne).retirerJeton(idRangee);
	...
	public boolean siPossibleJouerIci(int indiceColonne, int indiceRangee) {
		...
		if(siIndiceColonneValide(indiceColonne)){
			...
			siPossible = colonnes.get(indiceColonne).siPossibleJouerIci(indiceRangee);
		}
